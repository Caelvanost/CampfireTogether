...
Current branch target: **v0.1.1 prototype**.
...
- suppresses a remotely replicated `Build Campfire` magic effect before it starts a second local placement flow;
...
v0.1.1 intentionally ships two targeted Campfire script overrides:

- `Scripts/CampConjureObjectEffect.pex` — ignores a replicated Build Campfire effect when its caster/target is not the local player;
...
- v0.1.1 sends runtime base FormIDs and therefore assumes identical load order on both PCs.
...
- `build/CampfireTogether-v0.1.1.zip`.
