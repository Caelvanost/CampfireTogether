...
Current branch target: **v0.1.2 prototype**.
...
- suppresses a remotely replicated `Build Campfire` magic effect before it starts a second local placement flow, even when STR makes Papyrus see the local player as caster/target;
...
v0.1.2 intentionally ships two targeted Campfire script overrides:

- `Scripts/CampConjureObjectEffect.pex` — allows placement only when the DLL consumes a recent local `Shout`/Power input intent;
...
- v0.1.2 sends runtime base FormIDs and therefore assumes identical load order on both PCs.
...
- `build/CampfireTogether-v0.1.2.zip`.
