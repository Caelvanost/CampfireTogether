...
namespace STRPM
{
    ...
    using ReceiveCallback = void(STRPM_CALL*)(const Message* message, void* userData);
    using LogCallback = void(STRPM_CALL*)(const char* message, void* userData);
    using ProxyMappingCallback = void(STRPM_CALL*)(const ProxyMappingEvent* event, void* userData);

    struct Interface
    {
        std::uint32_t version{ kInterfaceVersion };
        Result(STRPM_CALL* registerChannel)(const char* channel, ReceiveCallback callback, void* userData, ListenerHandle* outHandle);
        Result(STRPM_CALL* unregisterChannel)(ListenerHandle handle);
        Result(STRPM_CALL* send)(const char* channel, Target target, const void* data, std::size_t size, std::uint32_t flags);
        Result(STRPM_CALL* getLocalConnectionID)(ConnectionID* outConnectionID);
        Result(STRPM_CALL* setLogCallback)(LogCallback callback, void* userData);
    ...
    struct TransportInterface
    {
        ...
        Result(STRPM_CALL* start)(ReceiveCallback callback, void* userData);
    ...
    struct ProxyResolverInterface
    {
        ...
        Result(STRPM_CALL* resolve)(ConnectionID connectionID, ProxyFormID* outFormID);
        Result(STRPM_CALL* registerListener)(ProxyMappingCallback callback, void* userData);
        Result(STRPM_CALL* unregisterListener)(ProxyMappingCallback callback, void* userData);
    };

    using QueryInterfaceFn = Result(STRPM_CALL*)(std::uint32_t requestedVersion, const Interface** outInterface);
    using QueryDiagnosticsFn = Result(STRPM_CALL*)(std::uint32_t requestedVersion, const DiagnosticsInterface** outInterface);
    using QueryTransportInterfaceFn = Result(STRPM_CALL*)(std::uint32_t requestedVersion, const TransportInterface** outInterface);
    using QueryProxyResolverFn = Result(STRPM_CALL*)(std::uint32_t requestedVersion, const ProxyResolverInterface** outInterface);
