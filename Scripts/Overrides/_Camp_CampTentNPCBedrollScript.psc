...
Event OnActivate(ObjectReference akActionRef)
	if !in_use
		...
		if (akActionRef as Actor) == PlayerRef
			if TentObject && CampfireTogetherNative.IsRemoteCampObject(TentObject)
				; A remote player's tent uses Campfire's follower bedroll as the
				; local multiplayer player's sleeping position. Keep the normal
				; follower-only behavior for every locally owned tent.
				in_use = true
				self.BlockActivation(false)
				utility.wait(0.2)
				CampfireTogetherNative.ReportRemoteBedrollAccess(self, TentObject)
